...
// upload multiple file function -- start
...
// upload multiple file function -- end
      ...
      if (mongoose.Types.ObjectId.isValid(req.query.project_id)) {
        ...
        if (isProjectAccessibleByUser === true) {
          ...
          columns["columns"].forEach(async (column_id) => {
            ...
            let tasks = await Column.findById(column_id, {
              tasks: 1,
              _id: 0,
            });
...
router.post("/tasks", isLoggedIn, async (req, res, next) => {
  try {
    let { title, description, label, labelType, multipleFilePath, expireAt } =
      ...
      const mappedMultipleFilePath = multipleFilePath.map(item => item.slice(8)).join(';');
    ...
    await sheets.updateProperties({ title: "KANBANT" });
